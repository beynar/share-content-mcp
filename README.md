...
`share-content-mcp` is a Cloudflare Worker that exposes a small MCP server for publishing and updating HTML or Markdown pages.

It stores shared content in a single SQLite-backed Durable Object, serves each page at a stable public URL, generates OG images, and lets MCP clients share, update, and search pages by id, title, or label.

## What this MCP does

- Shares raw HTML as public pages
...
- Lets clients update existing pages in place while keeping the same URL
- Exposes MCP tools for sharing content, updating pages, and searching pages
...
- `update_content`
  - Inputs: `id`, then `html` or `markdown`, plus optional `title` and `label`
  - Output: updated page metadata including `id`, `url`, `format`, `title`, `label`, `createdAt`, and `updatedAt`
- `search_pages`
...
- Public pages keep a stable URL and can be updated in place through the MCP.
- Page and OG responses use revalidation headers (`ETag` and `Last-Modified`) instead of immutable caching.
